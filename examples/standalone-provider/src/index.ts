...
import {
  ...
  extractApiKey,
  extractClientId,
} from "@fireblocks/eth-staking-eth-link-api-validator";
  ...
  if (method === "POST" && url.pathname === "/eth-link/validators/create") {
    ...
    if (!clientId) {
      sendError(
        ...
        400,
        "INVALID_REQUEST",
        "Missing or invalid x-client-id",
        requestId,
      );
      return;
    }
    ...
    const addressHex = parsed.data.withdrawalAddress
      .replace("0x", "")
      .toLowerCase();
    ...
    const hmacSignature = signResponse(
      requestId,
      timestamp,
      depositDataRoot,
      API_KEY,
